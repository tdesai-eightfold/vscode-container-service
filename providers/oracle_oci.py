...
from oci.container_instances.container_instance_client import ContainerInstanceClient
...
class OracleCloudProvider(CloudBaseClass):
    ...
    def __init__(
        ...
        self._container_client = ContainerInstanceClient(self._config)
    ...
    def _get_private_ip_from_vnic_id(self, vnic_id: str) -> Optional[str]:
        """Get private IP from VNIC ID via VirtualNetworkClient."""
        try:
    ...
    def get_or_create_vpc(self, subnet_id: Optional[str] = None) -> VpcInfo:
        """Get VPC info - OCI requires subnet_id to be provided."""
        sid = subnet_id or self._subnet_id
        if not sid:
            raise ValueError("subnet_id required for OCI. Set in provider config.")
    ...
    def create_instance(
        ...
        # Build image URL from registry base + image_name + tag (same pattern as Terraform)
    ...
    def list_instances(self, compartment_or_project: Optional[str] = None) -> list[InstanceInfo]:
        """List OCI container instances."""
        comp = compartment_or_project or self._compartment_id
        response = self._container_client.list_container_instances(
            compartment_id=comp
        )
        items = response.data.items if hasattr(response.data, "items") else (response.data or [])
        result = []
        for inst in items:
            # ContainerInstanceSummary has no vnics; use get_instance(id) for private_ip
            result.append(
                InstanceInfo(
                    id=inst.id,
                    name=inst.display_name or inst.id,
                    status=inst.lifecycle_state or "UNKNOWN",
                    private_ip=None,  # Call get_instance(id) for full details including IP
                    provider=self.provider_name,
                )
            )
        return result
    ...
    def get_instance(self, instance_id: str) -> Optional[InstanceInfo]:
        """Get OCI container instance by ID."""
        try:
    ...
    def create_workspace(
        ...
        vpc = self.get_or_create_vpc(subnet_id=self._subnet_id)
        dns_created = False
        try:
